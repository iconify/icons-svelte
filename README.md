# @iconify-svelte/ic

**Google Material Icons**

Author: [Material Design Authors](https://github.com/material-icons/material-icons)

License: [Apache 2.0](https://github.com/material-icons/material-icons/blob/master/LICENSE)

Browse all icons: [preview Google Material Icons on Iconify](https://icon-sets.iconify.design/ic/)

## Installation

```bash
npm install github:iconify/icons-svelte#ic
```

## Usage

For full documentation visit [Iconify website](https://iconify.design/docs/usage/svg-css/svelte/).

Few usage examples:

```svelte
<script lang="ts">
import SampleIcon from '@iconify-svelte/ic/baseline-notifications-active';
</script>

<SampleIcon />
```

To resize icon, set `width` and/or `height` parameters:

```svelte
<script lang="ts">
import SampleIcon from '@iconify-svelte/ic/outline-person-outline';
</script>

<SampleIcon height="1em" />
```

To change icon color, use style:

```svelte
<script lang="ts">
import SampleIcon from '@iconify-svelte/ic/twotone-videocam-off';
</script>

<SampleIcon style="color: red;" />
```

# @iconify-svelte/flag

Icon set prefix: **flag**

Icon set name: **Flag Icons**

Author: **Panayiotis Lipiridis**

License: **[MIT](https://github.com/lipis/flag-icons/blob/main/LICENSE)**

Repository: **https://github.com/lipis/flag-icons**

Browse all icons: **[Preview Flag Icons](https://icon-sets.iconify.design/flag/)**

## Installation

```bash
npm install github:iconify/icons-svelte#flag
```

## Usage

For full documentation visit [Iconify website](https://iconify.design/docs/usage/svg-css/svelte/).

Few usage examples:

```svelte
<script lang="ts">
import SampleIcon from '@iconify-svelte/flag/fr-1x1';
</script>

<SampleIcon />
```

To resize icon, set `width` and/or `height` parameters:

```svelte
<script lang="ts">
import SampleIcon from '@iconify-svelte/flag/de-1x1';
</script>

<SampleIcon height="1em" />
```

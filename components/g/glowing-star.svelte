<script>
import Icon from '@iconify/css-svelte';

/** @type {{width?: string; height?: string;}} */
let {width, height, ...props} = $props();

const viewBox = {"width":128,"height":128};
const content = `<path class="g4pcqtbgi"/><g class="dz-kf-l3n"><path class="pcnwi1ygb"/><path class="muypiibfg"/></g>`;
</script>
<Icon width={width} height={height} viewBox={viewBox} content={content} fallback="noto-v1:glowing-star" {...props}></Icon>
<style>
:global(.dz-kf-l3n) {
  fill: var(--svg-color--f79329, #f79329);
}

:global(.g4pcqtbgi) {
  fill: var(--svg-color--fcc21b, #fcc21b);
  d: path("m43.48 97.83l5.81-23.67L30.8 58.8l24.11-1.93L64 34.68l9.09 22.19L97.2 58.8L78.71 74.16l5.81 23.67L64 84.85z");
}

:global(.muypiibfg) {
  d: path("m89.46 77.7l26.84-22.29c.86-.71 1.2-1.87.86-2.93a2.76 2.76 0 0 0-2.39-1.91l-35.03-2.79l-13.2-32.23a2.75 2.75 0 0 0-2.54-1.7c-1.11 0-2.12.67-2.54 1.7l-13.2 32.23l-35.03 2.79c-1.12.09-2.05.84-2.39 1.91s0 2.22.86 2.93L38.55 77.7l-8.44 34.37c-.26 1.08.16 2.22 1.06 2.88c.91.65 2.13.69 3.07.09L64 96.2l29.76 18.83c.94.59 2.16.56 3.07-.09c.9-.65 1.33-1.79 1.06-2.88zM43.48 97.83l5.82-23.67L30.8 58.8l24.11-1.93L64 34.68l9.09 22.19L97.2 58.8L78.71 74.16l5.81 23.67L64 84.85z");
}

:global(.pcnwi1ygb) {
  d: path("M10.66 28.38c2.85 1.66 5.66 3.39 8.55 5c3.96 2.22 7.96 4.54 12.15 6.27c1.47.61 2.65-.45 3.24-1.7c.64-1.37-.11-2.3-.74-3.51c-1.64-3.13-3.21-6.3-5.09-9.29c-1.9-3.04-3.36-6.38-5.6-9.2l-.2-.24c-1.11-1.4-2.49-3.31-4.47-3.15c-1.92.16-3.11 1.77-4.48 2.95c-2.31 1.98-4.13 4.56-5.65 7.17c-.64 1.11-1.36 2.6-.48 3.82c.64.86 1.87 1.36 2.77 1.88m13.35 60.23c-3.2.74-6.43 1.41-9.58 2.41c-3.17 1-6.53 1.53-9.61 2.89l-.25.12C3.04 94.7.99 95.5.68 97.32c-.31 1.78.87 3.22 1.61 4.73c1.24 2.55 3.14 4.8 5.14 6.78c.86.85 2.03 1.84 3.33 1.32c.94-.36 1.68-1.37 2.34-2.05c2.17-2.18 4.4-4.32 6.52-6.54c2.94-3.05 5.96-6.11 8.51-9.48c.89-1.19.22-2.5-.78-3.32c-1.08-.89-2.1-.43-3.34-.15m72.63-48.96c4.19-1.74 8.18-4.06 12.15-6.27c2.88-1.61 5.7-3.34 8.56-5c.89-.52 2.13-1.02 2.76-1.89c.89-1.22.17-2.7-.48-3.82c-1.51-2.61-3.34-5.19-5.65-7.17c-1.37-1.18-2.55-2.79-4.48-2.95c-1.97-.16-3.36 1.75-4.47 3.15l-.2.24c-2.25 2.83-3.7 6.16-5.61 9.2c-1.87 3-3.45 6.16-5.09 9.29c-.63 1.22-1.38 2.15-.74 3.51c.59 1.26 1.78 2.32 3.25 1.71m26.8 54.39l-.26-.12c-3.07-1.37-6.43-1.89-9.61-2.89c-3.15-1-6.37-1.67-9.58-2.41c-1.25-.28-2.27-.74-3.35.16c-.99.82-1.67 2.13-.78 3.32c2.55 3.37 5.59 6.43 8.51 9.48c2.13 2.22 4.35 4.36 6.52 6.54c.67.69 1.41 1.69 2.35 2.05c1.3.52 2.47-.48 3.33-1.32c2-1.98 3.91-4.23 5.15-6.78c.74-1.52 1.91-2.96 1.61-4.73c-.32-1.83-2.36-2.63-3.89-3.3");
}

</style>

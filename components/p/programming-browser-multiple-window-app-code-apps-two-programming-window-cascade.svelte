<script>
import Icon from '@iconify/css-svelte';
import '../../css/y/y9tr6bcfx.css';
import '../../css/e/e1ho-ybli.css';
import '../../css/g/gndeg6jzc.css';
import '../../css/g/g5n0smnba.css';

/** @type {{width?: string; height?: string;}} */
let {width, height, ...props} = $props();

const viewBox = {"width":14,"height":14};
const content = `<g class="y9tr6bcfx"><path class="e1ho-ybli"/><rect class="gndeg6jzc"/><path class="g5n0smnba"/></g>`;
</script>
<Icon width={width} height={height} viewBox={viewBox} content={content} fallback="streamline:programming-browser-multiple-window-app-code-apps-two-programming-window-cascade" {...props}></Icon>

<script>
import Icon from '@iconify/css-svelte';

/** @type {{width?: string; height?: string;}} */
let {width, height, ...props} = $props();

const viewBox = {"width":24,"height":24};
const content = `<path class="cde7yrb9q"/>`;
</script>
<Icon width={width} height={height} viewBox={viewBox} content={content} fallback="material-symbols-light:celebration-outline-rounded" {...props}></Icon>
<style>
:global(.cde7yrb9q) {
  fill: currentColor;
  d: path("m5.242 18.7l7.05-2.5l-4.55-4.55zM21.66 7.525q-.13.148-.304.148t-.304-.129l-.056-.056q-.465-.465-1.105-.465q-.641 0-1.106.465l-5.056 5.056q-.129.129-.304.139t-.323-.139t-.148-.313t.148-.314l5.056-5.055q.703-.704 1.713-.704t1.714.704l.074.075q.13.129.13.294t-.13.294M9.783 4.775q.148-.148.313-.148t.313.148l.299.298q.8.8.8 1.939t-.8 1.938l-.287.287q-.129.129-.304.138t-.323-.138t-.148-.314t.148-.313l.287-.287q.542-.542.542-1.311T10.081 5.7l-.298-.298q-.13-.129-.139-.304t.139-.323m3.665-1.462q.148-.148.314-.148q.165 0 .313.148l1.056 1.056q.723.723.723 1.733t-.723 1.732l-3.056 3.057q-.129.128-.304.138t-.323-.138t-.148-.314t.148-.313l3.056-3.056q.465-.466.465-1.106t-.465-1.106l-1.056-1.055q-.129-.13-.138-.304q-.01-.175.138-.323m6.23 10.884q-.147.148-.312.148q-.166 0-.314-.148l-.479-.479q-.561-.561-1.356-.561t-1.355.561l-.48.479q-.128.129-.303.139t-.323-.139t-.148-.313t.148-.314l.479-.479q.819-.819 1.982-.819t1.983.82l.479.478q.129.129.138.304t-.138.323M4.162 18.731l2.775-7.712q.105-.267.31-.404q.205-.136.447-.136q.162 0 .298.046t.268.177l4.98 4.98q.131.132.177.268q.047.137.047.298q0 .243-.137.447t-.404.31l-7.711 2.776q-.243.086-.47.028q-.226-.059-.388-.22q-.162-.162-.22-.389q-.059-.227.028-.47");
}

</style>

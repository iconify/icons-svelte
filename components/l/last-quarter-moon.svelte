<script>
import Icon from '@iconify/css-svelte';

/** @type {{width?: string; height?: string;}} */
let {width, height, ...props} = $props();

const viewBox = {"width":128,"height":128};
const content = `<path class="itj78k8ts"/><path class="qs67c0bml"/><path class="d_04kubro"/><path class="d28pz-btn"/><path class="ct33wgsxg"/><path class="zsyou9bob"/><path class="dikcdxr6r"/>`;
</script>
<Icon width={width} height={height} viewBox={viewBox} content={content} fallback="noto:last-quarter-moon" {...props}></Icon>
<style>
:global(.ct33wgsxg) {
  fill: var(--svg-color--f1c688, #f1c688);
  d: path("M24.24 102.04c.11-.16.99-.39 2.01.57c1.02.95 4.03 3.79 5.23 4.88s.03 2.01-.97 1.83c-4.8-.84-7.42-5.61-6.27-7.28m17.8 12.38c.05-.19.79-.71 2.08-.19s5.12 2.11 6.63 2.7s.74 1.86-.25 2.06c-4.8.92-8.94-2.6-8.46-4.57m6.26-3.58c.6.47 1.21 1.21.93 1.91c-.26.64-1.08.81-1.77.83c-2.28.07-4.56-.51-6.55-1.6c-1.4-.77-5.07-3.14-4.55-5.13c.74-2.82 10.4 2.8 11.94 3.99");
}

:global(.d_04kubro) {
  fill: var(--svg-color--f1c688, #f1c688);
  d: path("M62.34 8.58c-.02 1.36-3.4 2.23-7.38 2.6s-7.3.11-7.65-1.2S50 6.72 54.34 6.3s8.01.92 8 2.28");
}

:global(.d28pz-btn) {
  fill: var(--svg-color--1b2e35, #1b2e35);
  d: path("M119.37 59.47c-.01-.4.01-.8.08-1.21c1.22-7.23 1.8-14-6.32-18.14c-.89-.46-1.87-.83-2.55-1.57c-1.25-1.35-1.14-3.41-1.46-5.22c-.97-5.54-7.24-9.51-12.66-8.01c-1.78.49-3.64 1.47-5.35.8c-1.38-.54-2.16-1.97-2.94-3.23c-2.21-3.58-5.6-6.76-9.77-7.34s-8.91 2.3-9.17 6.5c-.11 1.83.2 3.99 1.6 5.18c5.88 5.01 9.52 1.32 12.25 3.91c3.17 3.01-3.41 3.82-2.32 11.47c.55 3.82 4.34 7.4 8.19 7.08c1.71-.15 4.31-.24 5.26 1.19c.75 1.12.56 2.19.56 4.03c0 4.54.5 8.11 4.92 9.14c.57.13 1.19.2 1.72-.03c1.17-.51 1.16-2.08 1.08-3.36c-.35-5.62-4.31-6.76-5.03-9.85c-.91-3.94 4.47-9.27 8.87-7.78c3.5 1.19.65 5.25 3.21 9.94c1.69 3.11 5.01 4.54 6.24 7.08s.09 4.3.69 5.91c.93 2.52 2.41 2.56 3.24 1.67c.91-.97.61-2.96.39-4.12c-.24-1.35-.69-2.67-.73-4.04");
}

:global(.dikcdxr6r) {
  fill: var(--svg-color--f1c688, #f1c688);
  d: path("M56.47 15.47c-9.5-1.73-12.9 3.96-15.81 5.2c-2.32.98-3.92-.73-7.5 1.46c-4.94 3.01-3.07 5.19-3.52 7.74c-.46 2.55-3.19 3.64-5.19 3.83c-2.37.23-2.56-1.21-1.42-2.94c.63-.95 1.3-1.54 1.62-2.66c.37-1.3.17-2.7.89-3.9c1.13-1.88 3.77-2.14 5.4-3.6c1.18-1.06 1.75-2.69 3.01-3.66s2.9-1.32 4.47-1.6c3.2-.57 5.9-3.09 4.45-3.17c-1.46-.08-3.89-.55-5.93.95c-2.27 1.66-5.32 2.18-7.73 3.55c-2.7 1.55-4.12 4.92-4.94 5.83s-3.46 1-5.28 2.88C15.19 29.27 8.85 43.4 8.85 48.69c0 4.93 2.19 9.75 7.08 11.09c.74.2 1.53.43 2.01 1.03c1.1 1.37-1.65 5.49-2.07 7.15c-1.07 4.3.1 7.61 2 9.98c4.11 5.11 8.21 4.42 12.33 1.89c1-.62 1.86-1.49 2.97-1.9c2.34-.87 2.4.63 2.63 2.38c1.08 8.06 4.43 10.14 6.78 11.05c4.89 1.89 10.9.09 14.08-4.08s3.53-10.4.66-14.79c-4.38-6.7-13.03-7.07-14.78-10.82c-1.97-4.22 3.48-4.81 5.86-8.47c2.12-3.25 2.56-7.72.54-11.08c-1-1.66-2.66-3.32-2.24-5.21c.47-2.11 6.48-5.42 10.95-9.26c3.18-2.72 10.63-10.03-1.18-12.18M26.01 48.86c-.34.65-.85 1.3-1.58 1.41c-.69.1-1.35-.34-1.78-.89c-.68-.89-.88-2.14-.45-3.17c.51-1.24 2.25-2.23 3.6-1.65c.73.31.95 1.05.91 1.86c-.05.89-.4 1.86-.7 2.44m15.5 2.94c-.14.79-.39 1.55-.57 2.07c-.49 1.45-1.2 2.97-2.57 3.64c-.6.3-1.27.4-1.93.51c-1.37.21-3.08.28-3.79-.91c-.26-.43-.32-.95-.37-1.45c-.23-2.16-.34-4.67 1.25-6.14c1.14-1.06 2.42-.93 3.84-.94c.8 0 2.73-.26 3.41.18c.91.59.94 1.85.73 3.04");
}

:global(.itj78k8ts) {
  fill: var(--svg-color--fce5ac, #fce5ac);
  d: path("M4 64c0 33.14 26.86 60 60 60c0 0 2.94-18.45 2.96-60.41S64 4 64 4C30.86 4 4 30.86 4 64");
}

:global(.qs67c0bml) {
  fill: var(--svg-color--22404c, #22404c);
  d: path("M124 64c0-33.14-26.86-60-60-60v120c33.14 0 60-26.86 60-60");
}

:global(.zsyou9bob) {
  fill: var(--svg-color--1b2e35, #1b2e35);
  d: path("M104.3 96.51c-1.34 1.59-4.84 5.56-6.01 6.92c-1.18 1.35-.8 2.45-.58 2.57c2.32 1.33 8.41-2.29 9.1-8.4c.15-1.27-1.18-2.68-2.51-1.09m7.34-11.29c-.62 1.56-2.29 5.51-2.85 6.84c-.56 1.34.01 2.08.21 2.12c2.12.43 5.87-3.9 4.84-8.77c-.2-1-1.58-1.75-2.2-.19M77.82 8.69c-1.01.24-1.77 1.64-.22 2.22s5.47 2.16 6.8 2.68s2.08-.07 2.12-.27c.45-2.14-3.83-5.78-8.7-4.63");
}

</style>

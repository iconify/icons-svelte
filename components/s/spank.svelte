<script>
import Icon from '@iconify/css-svelte';

/** @type {{width?: string; height?: string;}} */
let {width, height, ...props} = $props();

const viewBox = {"width":24,"height":24};
const content = `<g class="rxjxngljy"><path class="k6b-vzbpu"/><path class="qtozxqyfd"/><path class="h6b6fjbgp"/></g>`;
</script>
<Icon width={width} height={height} viewBox={viewBox} content={content} fallback="token-branded:spank" {...props}></Icon>
<style>
:global(.h6b6fjbgp) {
  d: path("M11.767 13.204q0-.444-.313-.68q-.314-.24-1.128-.5q-.816-.267-1.29-.523q-1.295-.699-1.295-1.884q0-.617.344-1.097q.35-.486.998-.757a3.8 3.8 0 0 1 1.462-.272q.815 0 1.451.298q.638.292.987.83q.355.538.355 1.222h-1.566q0-.522-.329-.81q-.328-.292-.924-.292q-.574 0-.893.246a.76.76 0 0 0-.319.637q0 .37.37.621q.378.25 1.103.47q1.336.402 1.948.997q.61.596.61 1.483q0 .987-.746 1.551q-.747.558-2.01.559a3.9 3.9 0 0 1-1.599-.319q-.72-.324-1.1-.882a2.26 2.26 0 0 1-.377-1.295h1.572q0 1.258 1.503 1.258q.56 0 .873-.225a.75.75 0 0 0 .313-.636");
}

:global(.k6b-vzbpu) {
  d: path("m16.545 8.543l-1.506 5.51l-1.245-.088l.939-5.68zm-1.314 6.084l-.392 1.946l-1.654-.482l.379-1.867z");
}

:global(.qtozxqyfd) {
  d: path("M5.921 17.024v.116h-1.19v-2.182l-.037.12l-1.138-.347l.643-2.103l-.079.117l-.986-.665l1.232-1.828l-.117.096l-.749-.925l1.774-1.437l-.227.087l-.426-1.11l2.119-.814l-.248.017l-.083-1.187l2.218-.155l-.2-.046l.268-1.16l2.119.49l-.134-.077L11.275 3l1.848 1.067l-.072-.077l.87-.812l1.44 1.544l-.03-.063l1.069-.522l.923 1.893l-.008-.053l1.175-.186l.347 2.187l.024-.156l1.175.186l-.351 2.219l.087-.178l1.069.521l-.99 2.03l.145-.154l.87.811l-1.539 1.65l.187-.107l.595 1.03l-1.934 1.117l.194-.045l.268 1.16l-2.137.493l.165.011l-.083 1.187l-2.139-.15l.109.043l-.426 1.11l-2.048-.786l.129.105l-.749.925l-1.69-1.369l.083.124l-.986.665l-1.208-1.79l.036.117l-1.139.348zm0-1.414l.743-.227l.578 1.89l.643-.434l1.098 1.628l.49-.605l1.523 1.232l.273-.71l1.834.705l.054-.768l1.962.137l-.176-.765l1.924-.445l-.399-.69l1.718-.991l-.589-.549l1.357-1.455l-.724-.353l.873-1.79l-.792-.124l.311-1.964l-.757.12l-.31-1.965l-.682.333l-.867-1.776l-.556.518l-1.34-1.437l-.383.664l-1.697-.98l-.175.76l-1.91-.44l.056.792l-1.96.137l.29.755l-1.845.708l.496.613l-1.548 1.254l.653.441l-1.117 1.656l.753.23l-.583 1.908h.781z");
}

:global(.rxjxngljy) {
  fill: var(--svg-color--ff3b81, #ff3b81);
}

</style>

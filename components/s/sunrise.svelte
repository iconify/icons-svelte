<script>
import Icon from '@iconify/css-svelte';

/** @type {{width?: string; height?: string;}} */
let {width, height, ...props} = $props();

const viewBox = {"width":64,"height":64};
const content = `<path class="bps0vnbab"/><path class="xwgu4jb-l"/><path class="wxb-w0umm"/><path class="mibtahhay"/><path class="moo-nkssr"/><path class="d53_nmbzq"/><path class="wv780abrz"/><path class="nvjxv8cax"/><path class="i_4a4v5lt"/><path class="susqlpbrb"/>`;
</script>
<Icon width={width} height={height} viewBox={viewBox} content={content} fallback="emojione-v1:sunrise" {...props}></Icon>
<style>
:global(.bps0vnbab) {
  fill: var(--svg-color--fbb041, #fbb041);
  d: path("M63.716 56.818a6.84 6.84 0 0 1-6.842 6.846H7.101a6.84 6.84 0 0 1-6.838-6.846V7.049A6.84 6.84 0 0 1 7.101.208h49.775a6.84 6.84 0 0 1 6.84 6.841z");
}

:global(.d53_nmbzq) {
  fill: var(--svg-color--85bde8, #85bde8);
  d: path("M.262 41.59h63.45V61.6H.262z");
}

:global(.i_4a4v5lt) {
  fill: var(--svg-color--fbed21, #fbed21);
  d: path("M36.421 50.584c-8.836-.262-17.474-1.881-26.19-3.02a21.4 21.4 0 0 0 4.394 6.33c10.829-1.107 21.615-2.865 32.476-3.035c.161-.234.294-.492.45-.734c-3.669.366-7.36.575-11.127.464m-4.88 7.611c3.568-1.5 7.19-2.92 11.04-3.504c.429-.064.857-.115 1.282-.176c.936-.85 1.81-1.756 2.583-2.758c-10.413.055-20.765 1.639-31.14 2.787a21.2 21.2 0 0 0 11.956 5.393A171 171 0 0 0 31.55 58.2");
  opacity: var(--svg-opacity--0-52, 0.52);
}

:global(.mibtahhay) {
  fill: var(--svg-color--f7ed4c, #f7ed4c);
  d: path("m6.347 32.21l-6.06-1.689v4.281l5.383.727c.15-1.121.381-2.238.68-3.319m48.96 6.724c0 1.156-.081 2.311-.232 3.434l7.212.984a7 7 0 0 0 1.444-2.881v-1.648H55.31zM13.333 20.729L.293 6.797c-.003.084-.013.167-.013.252v7.47l10.719 8.713a25 25 0 0 1 2.33-2.503M9.03 26.02L.282 20.699v5.218l7.188 3.115a24 24 0 0 1 1.56-3.01M5.674 42.39a25 25 0 0 1-.234-3.42l-5.155.002v1.463a6.9 6.9 0 0 0 1.133 2.539zM53.25 28.966l10.491-4.581v-5.793l-12.05 7.373c.587.96 1.113 1.972 1.561 3M50.36.208l-8.556 16.546c1 .518 1.974 1.113 2.904 1.765L57.594.243a7 7 0 0 0-.692-.035zm-.653 22.972l14.03-11.46V7.052a6.8 6.8 0 0 0-.762-3.128L47.368 20.685a26 26 0 0 1 2.338 2.495m5.374 12.263l8.661-1.195v-4.736l-9.351 2.634a25 25 0 0 1 .69 3.297M38.7 15.413L44.077.209h-5.696L35.45 14.506c1.096.227 2.191.531 3.25.907m-13.47-.892L22.25.208h-5.696l5.436 15.227a24 24 0 0 1 3.24-.914m5.144-.522c.585-.021 1.137.018 1.699.055L32.986.208H27.65l.953 13.851a26 26 0 0 1 1.771-.06m-11.478 2.786L10.262.208h-3.14a6.8 6.8 0 0 0-3.466.951l12.333 17.402a25 25 0 0 1 2.907-1.776");
}

:global(.moo-nkssr) {
  fill: var(--svg-color--368eba, #368eba);
  d: path("M28.468 44.44c-11.266 3.852-22.19 4.246-28.21 4.08V60.6c0 3.779 3.061 3.063 6.838 3.063h49.775c3.78 0 6.84.428 6.84-3.352V44.442h-35.25");
}

:global(.nvjxv8cax) {
  fill: var(--svg-color--fbed21, #fbed21);
  d: path("M30.21 44.16c-4.488-.742-8.945-1.746-13.418-2.566H8.496c.22 1.664.645 3.262 1.228 4.781c8.774 1.107 17.472 2.963 26.328 3.227c4.092.123 8.101-.078 12.08-.439a21.6 21.6 0 0 0 1.529-3.375c-6.515-.05-12.848-.528-19.45-1.628");
  opacity: var(--svg-opacity--0-52, 0.52);
}

:global(.susqlpbrb) {
  fill: var(--svg-color--fbed21, #fbed21);
  d: path("M2.702 58.5c.337.02.669.051 1.01.051c4.543 0 8.703-1.57 12.01-4.172c-3.971.789-12.539 3.922-13.02 4.121m32.272-15.838c.313.127.616.266.938.375c4.296 1.479 8.741 1.346 12.713-.039c-4.01-.547-13.13-.369-13.651-.336M27.865 61.55c-.336.02-.671.049-1.01.049c-4.544 0-8.703-1.57-12.01-4.172c3.968.789 12.539 3.922 13.02 4.123M61.7 48.51c-.241.016-.479.037-.72.037c-3.226 0-6.18-1.113-8.53-2.961c2.827.564 8.908 2.783 9.25 2.924m-12.741 3.87c.118.004.241.018.36.018a6.85 6.85 0 0 0 4.261-1.48c-1.409.282-4.447 1.39-4.621 1.462M2.246 46c.124.004.238.02.364.02a6.9 6.9 0 0 0 4.261-1.48C5.46 44.818 2.42 45.928 2.246 46");
  opacity: var(--svg-opacity--0-52, 0.52);
}

:global(.wv780abrz) {
  fill: var(--svg-color--368eba, #368eba);
  d: path("M41.864 53.3c-13.246.891-29.29 1.775-41.602-4.4v12.773h63.45V47.675c-6.662 3.51-14.626 5.137-21.852 5.625");
}

:global(.wxb-w0umm) {
  fill: var(--svg-color--ff0, #ff0);
  d: path("M30.377 17.05c-12.09 0-21.879 9.799-21.879 21.879c0 2.416.407 4.734 1.128 6.906h41.501a21.9 21.9 0 0 0 1.132-6.906c0-12.08-9.797-21.879-21.882-21.879");
}

:global(.xwgu4jb-l) {
  fill: var(--svg-color--fbed21, #fbed21);
  d: path("M49.944 44.541c.288-.955.526-1.934.658-2.951h-25.2c.883.174 17.193 2.781 24.54 2.951M30.23 58.5c.336.02.667.051 1.01.051c4.542 0 8.7-1.57 12.01-4.172c-3.978.789-12.545 3.922-13.02 4.121");
  opacity: var(--svg-opacity--0-52, 0.52);
}

</style>

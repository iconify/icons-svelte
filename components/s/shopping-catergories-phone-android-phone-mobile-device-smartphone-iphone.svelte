<script>
import Icon from '@iconify/css-svelte';
import '../../css/y/y9tr6bcfx.css';
import '../../css/u/uo5h1db1c.css';
import '../../css/n/nrkc6qbqt.css';
import '../../css/p/pamn5g9ib.css';

/** @type {{width?: string; height?: string;}} */
let {width, height, ...props} = $props();

const viewBox = {"width":14,"height":14};
const content = `<g class="y9tr6bcfx"><rect class="uo5h1db1c"/><path class="nrkc6qbqt"/><circle class="pamn5g9ib"/></g>`;
</script>
<Icon width={width} height={height} viewBox={viewBox} content={content} fallback="streamline:shopping-catergories-phone-android-phone-mobile-device-smartphone-iphone" {...props}></Icon>

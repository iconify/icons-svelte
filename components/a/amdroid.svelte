...
const content = `<circle class="tsa4-2-ul"/><path class="h-h5b9dfo"/><circle class="fd85t5b_y"/><path class="eq9z-9bbu"/>`;
</script>
<Icon width={width} height={height} viewBox={viewBox} content={content} fallback="arcticons:amdroid" {...props}></Icon>
<style>
:global(.eq9z-9bbu) {
  fill: none;
  stroke: currentColor;

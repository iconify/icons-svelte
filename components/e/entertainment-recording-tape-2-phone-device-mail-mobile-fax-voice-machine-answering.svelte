<script>
import Icon from '@iconify/css-svelte';
import '../../css/y/y9tr6bcfx.css';
import '../../css/u/ucsug7a9v.css';
import '../../css/o/o3jp4gbln.css';
import '../../css/q/q0nz_6bfb.css';

/** @type {{width?: string; height?: string;}} */
let {width, height, ...props} = $props();

const viewBox = {"width":14,"height":14};
const content = `<g class="y9tr6bcfx"><circle class="ucsug7a9v"/><circle class="o3jp4gbln"/><path class="q0nz_6bfb"/></g>`;
</script>
<Icon width={width} height={height} viewBox={viewBox} content={content} fallback="streamline:entertainment-recording-tape-2-phone-device-mail-mobile-fax-voice-machine-answering" {...props}></Icon>

<script>
import Icon from '@iconify/css-svelte';
import '../../css/f/ft5dv1b6b.css';
import '../../css/d/djqetsxsa.css';
import '../../css/o/odi669bij.css';
import '../../css/n/ntrj6wbus.css';
import '../../css/l/lc00ugj9n.css';

/** @type {{width?: string; height?: string;}} */
let {width, height, ...props} = $props();

const viewBox = {"width":256,"height":256};
const content = `<g class="ft5dv1b6b"><rect class="djqetsxsa"/><path fill="url(#SVGrL22Wceb)" class="odi669bij"/><defs><radialGradient id="SVGrL22Wceb" cx="0" cy="0" r="1" gradientTransform="matrix(0 199.78 -230.653 0 21.293 125.178)" gradientUnits="userSpaceOnUse"><stop class="ntrj6wbus"/><stop offset="1" class="lc00ugj9n"/></radialGradient></defs></g>`;
</script>
<Icon width={width} height={height} viewBox={viewBox} content={content} fallback="skill-icons:netlify-light" {...props}></Icon>
